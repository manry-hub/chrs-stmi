import webpush from 'web-push';
import { adminDb } from '../firebase/admin';

const VAPID_PUBLIC_KEY = process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY || '';
const VAPID_PRIVATE_KEY = process.env.VAPID_PRIVATE_KEY || '';

if (VAPID_PUBLIC_KEY && VAPID_PRIVATE_KEY) {
  webpush.setVapidDetails(
    'mailto:admin@stmi.ac.id',
    VAPID_PUBLIC_KEY,
    VAPID_PRIVATE_KEY
  );
}

/**
 * Send push notification exclusively to all superadmins.
 */
export async function sendPushToSuperadmins(payload: { title: string; body: string; url?: string }) {
  if (!VAPID_PUBLIC_KEY || !VAPID_PRIVATE_KEY) {
    console.error('❌ VAPID Keys belum diset! Notifikasi tidak dikirim.');
    return;
  }

  try {
    const snapshot = await adminDb
      .collection('pushSubscriptions')
      .where('role', '==', 'superadmin')
      .get();

    if (snapshot.empty) {
      console.log('No superadmin push subscriptions found.');
      return;
    }

    const notifications = snapshot.docs.map(async (doc) => {
      const data = doc.data();
      const subscription = data.subscription;
      try {
        await webpush.sendNotification(
          subscription,
          JSON.stringify(payload)
        );
      } catch (error: unknown) {
        if (error !== null && typeof error === 'object' && 'statusCode' in error) {
          const pushError = error as { statusCode: number };
          if (pushError.statusCode === 410 || pushError.statusCode === 404) {
            console.log(`Push subscription for superadmin ${doc.id} expired. Removing.`);
            await doc.ref.delete();
          } else {
            console.error(`Error sending push to ${doc.id}:`, error);
          }
        } else {
          console.error(`Error sending push to ${doc.id}:`, error);
        }
      }
    });

    await Promise.allSettled(notifications);
  } catch (error) {
    console.error('Error in sendPushToSuperadmins:', error);
  }
}
